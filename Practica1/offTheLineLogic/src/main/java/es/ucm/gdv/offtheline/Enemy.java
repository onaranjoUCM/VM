...
public class Enemy extends GameObject {
    ...
    public void update(double deltaTime) {
        if(offset_.x != 0 || offset_.y != 0) {
            ...
            float incY = 0;
            if(!stop_) {
                incX = (float) (moveSpeed_ * deltaTime * dir_.x);
                incY = (float) (moveSpeed_ * deltaTime * dir_.y);
            }
            posX_ += incX;
            posY_ += incY;
            vertexA.x += incX;
    ...
    private void updateDirection(double deltaTime) {
        if((dir_.x != 0 && posX_ * dir_.x >= offset_.x * dir_.x) || (dir_.y != 0 && posY_ * dir_.y >= offset_.y * dir_.y)){
            if(!stop_) {
                dir_.x = -dir_.x;
                dir_.y = -dir_.y;
